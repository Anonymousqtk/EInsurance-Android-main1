...
public class GetLoaiXe extends AbstractProgressableAsyncTask<SoapObject, ArrayList<LoaiXeObject>> {
    private static final String TAG = GetLoaiXe.class.getName();
    private static final String WS_METHOD_NAME = "Dm_loaixe_auto_new";

    public GetLoaiXe() {}

    public static SoapObject createRequest(String mucdichsd_val) {
        ...
        request.addProperty("parent_val", mucdichsd_val);
        return request;
    }

    @Override
    protected ArrayList<LoaiXeObject> performTaskInBackground(SoapObject parameter) throws Exception {
        ...
        ArrayList<LoaiXeObject> result = new ArrayList<>();
        try {
            ...
            if (!jsonValue.isNull("Consumers")) {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    Gson gson = new Gson();
                    LoaiXeObject resume = gson.fromJson(currentObject.toString(), LoaiXeObject.class);
                    result.add(resume);
                }
            }
