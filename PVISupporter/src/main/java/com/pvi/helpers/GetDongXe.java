...
public class GetDongXe extends AbstractProgressableAsyncTask<SoapObject, ArrayList<DongXeObject>> {
    private static final String TAG = GetDongXe.class.getName();
    private static final String WS_METHOD_NAME = "Dm_dongxe_auto_new";

    public GetDongXe() {}

    public static SoapObject createRequest(String parent_val) {
        ...
        request.addProperty("parent_val", parent_val);
        return request;
    }

    @Override
    protected ArrayList<DongXeObject> performTaskInBackground(SoapObject parameter) throws Exception {
        ...
        ArrayList<DongXeObject> result = new ArrayList<>();
        try {
            ...
            if (!jsonValue.isNull("Consumers")) {
                ...
                for (int i = 0; i < array.length(); i++) {
                    ...
                    Gson gson = new Gson();
                    DongXeObject resume = gson.fromJson(currentObject.toString(), DongXeObject.class);
                    result.add(resume);
                }
            }
